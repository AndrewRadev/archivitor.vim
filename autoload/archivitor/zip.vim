...
function! archivitor#zip#Filelist() dict
  ...
  for line in split(archivitor#util#SilentSystem('unzip -qql', self.name), "\n")
...
function! archivitor#zip#Extract(...) dict
  call archivitor#util#System('unzip -o', self.name, a:000)
endfunction

function! archivitor#zip#Update(...) dict
  call archivitor#util#System('zip -u', self.name, a:000)
endfunction

function! archivitor#zip#Delete(paths) dict
  call archivitor#util#System('zip', self.name, {'-d': a:paths})
endfunction

function! archivitor#zip#Add(path) dict
  call archivitor#util#System('zip', self.name, {'-r': a:path})
endfunction
