...
function! archivitor#tar#Filelist() dict
  let file_list = []
  for line in split(archivitor#util#System('tar -tf', self.name), "\n")
...
function! archivitor#tar#Extract(...) dict
  call archivitor#util#System('tar -xf', self.name, a:000)
endfunction
...
function! archivitor#tar#Update(...) dict
  ...
  if self.name =~ '\.tar$'
    call archivitor#util#System('tar -rf', self.name, files)
    return
  endif

  let cached_directory = s:CachedDir(self)
  call archivitor#util#System('cp -r', files, cached_directory)
...
function! archivitor#tar#Delete(paths) dict
  if self.name =~ '\.tar$'
    ...
    call archivitor#util#System('tar --delete -f', self.name, paths)
    return
  endif
  ...
  call archivitor#util#System('rm -r', paths)
  call s:UpdateFromCachedDir(self)
endfunction

function! archivitor#tar#Add(path) dict
  if !filereadable(self.name)
    call archivitor#util#System('tar -caf', self.name, a:path)
    return
  endif

  if self.name =~ '\.tar$'
    call archivitor#util#System('tar -rf', self.name, a:path)
    return
  endif

  let cached_directory = s:CachedDir(self)
  call archivitor#util#System('cp -r', a:path, cached_directory)
  call s:UpdateFromCachedDir(self)
endfunction
...
function! s:UpdateFromCachedDir(archive)
  ...
  let cwd = getcwd()
  exe 'cd '.directory
  call archivitor#util#System('tar -caf '.shellescape(a:archive.name).' *')
  exe 'cd '.cwd
endfunction
